...
$error=false;
 ...
 if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['btn-guardar'])) {
     $descripcion=strtoupper($_POST['descripcion_equipo']);
    ...
    $res=mysqli_query($conn,"SELECT * FROM tipo_equipo WHERE numeroinventario='$numero_inventario' ");
	   $row=mysqli_fetch_array($res);
	   $count = mysqli_num_rows($res); 
   
		if( $count != 0) {
			$error = true;
			$error_inventario = "El tipo de equipo $descripcion ya se encuentra registrado";
                        
                  
		} else{
                    
                     
            $sql="INSERT INTO inventario_it.tipo_equipo (descripcion,numeroinventario,fechacompra,numerofactura,nombreproveedor,codigomodelo,codigoestadomodelo) VALUES ('$descripcion','$numero_inventario','$fecha_compra','$numero_factura','$nombre_proveedor','$codigo_modelo','$codigo_estado')";
            
                    
                if (mysqli_query($conn, $sql)) {
                
                
                echo "<script languaje='javascript'>alert('El tipo de equipo $descripcion se registro correctamente');"
        ...
        <div class="col-md-12">
          ...
          <div class="block-web">
            <div class="header">
              ...
              <h3 class="content-header">Tipo de equipos</h3>
            </div>
         <div class="porlets-content">
          ...
          <div class="adv-table table-responsive ">
                          ...
                          <div class="clearfix">
                              <div class="btn-group">
                                  <a  class="btn btn-primary" data-toggle="modal" data-target="#tipo_equipo">
                          ...
                          <form name="myform" method="POST" action="tipo_equipo_excel.php">
                    ...
                    $sql = "SELECT TE.*,M.NOMBRE AS NOMBREMODELO, MA.NOMBRE AS NOMBREMARCA, E.ESTADO FROM tipo_equipo te
                            ...
                            INNER JOIN ESTADO E ON E.SECUENCIAL=TE.CODIGOESTADOMODELO";
                    ...
                    while($row = $query->fetch_assoc()){
                      ...
                      ?>
                                  ...
                                  <td><?= $row['NUMEROINVENTARIO'];?></td>
                                  <td><?=date("d/m/Y", strtotime($row['FECHACOMPRA']))?></td>
                                  ...
                                  <input name="descripcion[]" hidden="hidden"  value="<?=$row['DESCRIPCION']?>">
                                  <input name="numero_inventario[]" hidden="hidden"  value="<?=$row['NUMEROINVENTARIO']?>">
                                  <input name="fecha_compra[]" hidden="hidden"  value="<?=$row['FECHACOMPRA']?>"> 
                                  <input name="nombre_modelo[]" hidden="hidden"  value="<?=$row['NOMBREMODELO']?>">
                                  <input name="nombre_marca[]" hidden="hidden"  value="<?=$row['NOMBREMARCA']?>"> 
                                  <input name="numero_factura[]" hidden="hidden"  value="<?=$row['NUMEROFACTURA']?>">     
                                  <input name="nombre_proveedor[]" hidden="hidden"  value="<?=$row['NOMBREPROVEEDOR']?>">
                                  <input name="estado_equipo[]" hidden="hidden"  value="<?=$row['ESTADO']?>">    
                                                                                                    
                                  <td><a class="btn btn-info" onclick="return confirm('Desea modificar el registro?')"   href="modificar_equipo.php?id=<?= $row['SECUENCIAL'] ?>" >Editar</a></td>
                                  <td><a class="btn btn-danger" onclick="return confirm('Desea eliminar el registro?')" href="eliminar_equipo.php?id=<?= $row['SECUENCIAL'] ?>">Eliminar</a></td>
                              </tr>
                                         <?php  }
...
<div class="modal fade" id="tipo_equipo" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-body">
          ...
          <div class="form-group">
            <label for="recipient-name" class="col-form-label">Descripción:</label>
             <input type="text" name="descripcion_equipo" class="form-control" style="text-transform:uppercase;" required>
          </div>
           <div class="form-group">
            <label for="recipient-name" class="col-form-label"># Inventario:</label>
             <input type="text" name="numero_inventario" class="form-control" style="text-transform:uppercase;" required>
          </div>
              
              <div class="form-group">
            <label for="recipient-name" class="col-form-label">Fecha compra:</label>
             <input type="date" name="fecha_compra" class="form-control"  required>
          </div>
              
              <div class="form-group">
            <label for="recipient-name" class="col-form-label"># de factura:</label>
             <input type="text" name="numero_factura" class="form-control" style="text-transform:uppercase;" >
          </div>
              
               <div class="form-group">
            <label for="recipient-name" class="col-form-label">Proveedor:</label>
             <input type="text" name="nombre_proveedor" class="form-control" style="text-transform:uppercase;" >
          </div>
